...
//Modificar al nombre que se asigne en el dashboard.
  #define TEAM_NAME "ioht/oscar/002" //  proyecto/usuario/no.estacion 
  /*
  ioht/ugal/001
  ioht/oscar/001
  ioht/isidro/001
  ...
  */
  ...
  const char* cliendID= "ioht_oscar_2"; 
...
//Nombre del Servidor MQTT
  ...
  boolean datosEnMQTTServer = false;  //Asegurarse que datos en MQTT Server
...
void loop() {
  ...
  if(((timeClient.getMinutes() % 15 == 00) && (timeClient.getSeconds() > 00) && (timeClient.getSeconds() < 10))) {
    if (datosEnMQTTServer){
      ...
      Serial.println("Datos no publicados en MQTT Server");
      publicarDatos();
    }
  }
...
void reconnect() {
  ...
  while (!mqtt_client.connected()) {
    ...
    if (mqtt_client.connect(cliendID, user, passwd)) {
